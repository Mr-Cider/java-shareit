...
public interface BookingStorage extends JpaRepository<Booking, Long> {
    ...
    List<Booking> getUserFutureBookings(@Param ("userId") Long userId, @Param("status") BookingStatus status, @Param("date") LocalDate date);
    ...
    List<Booking> getUserCurrentBookings(@Param ("userId") Long userId, @Param("status") BookingStatus status, @Param("date") LocalDate date);

    @Query("SELECT b FROM Booking b " +
            "JOIN b.item i WHERE i.owner.id = :ownerId AND b.bookingStatus = :status AND (:date BETWEEN b.startDate AND b.endDate)")
    List<Booking> getOwnerCurrentBookings(@Param ("ownerId") Long ownerId, @Param("status") BookingStatus status, @Param("date") LocalDate date);

    @Query("SELECT b FROM Booking b " +
            "JOIN b.item i WHERE i.owner.id = :ownerId AND b.bookingStatus = :status AND b.endDate < :date ")
    List<Booking> getOwnerPastBookings(@Param ("ownerId") Long ownerId, @Param("status") BookingStatus status, @Param("date") LocalDate date);

    @Query("SELECT b FROM Booking b " +
            "JOIN b.item i WHERE i.owner.id = :ownerId AND b.bookingStatus = :status AND b.startDate > :date ")
    List<Booking> getOwnerFutureBookings(@Param ("ownerId") Long ownerId, @Param("status") BookingStatus status, @Param("date") LocalDate date);
