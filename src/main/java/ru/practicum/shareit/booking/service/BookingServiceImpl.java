...
public class BookingServiceImpl implements BookingService {
        ...
        switch (state) {
            case CURRENT -> {
                List<Booking> bookings = bookingStorage
                        .getUserCurrentBookings(userId, BookingStatus.APPROVED, LocalDate.now());
            ...
            case FUTURE -> {
                List<Booking> bookings = bookingStorage
                        .getUserFutureBookings(userId, BookingStatus.APPROVED, LocalDate.now());
                return convertToDtoList(bookings);
            }
            case WAITING -> {
    ...
    public List<BookingDto> getOwnerBookingState(Long ownerId, State state) {
        ...
        switch (state) {
            case CURRENT -> {
                List<Booking> bookings = bookingStorage
                        .getOwnerCurrentBookings(ownerId, BookingStatus.APPROVED, LocalDate.now());
                return convertToDtoList(bookings);
            }
            case PAST -> {
                List<Booking> bookings = bookingStorage
                        .getOwnerPastBookings(ownerId, BookingStatus.APPROVED, LocalDate.now());
                return convertToDtoList(bookings);
            }
            case FUTURE -> {
                List<Booking> bookings = bookingStorage
                        .getOwnerFutureBookings(ownerId, BookingStatus.APPROVED, LocalDate.now());
                return convertToDtoList(bookings);
            }
            case WAITING -> {
